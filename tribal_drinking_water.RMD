---
title: "Tribal Drinking Water - Data Processing"
output: 
  html_document:
...
Data on boundaries for tribal areas within California is available from multiple sources (we need to determine which is the 'official' dataset to use for SWRCB projects). Here are some potential sources:

1.   Census Bureau's American Indian / Alaska Native / Native Hawaiian Areas shapefile (can be accessed via the `tigris` R package) : "This shapefile contain both legal and statistical American Indian, Alaska Native, and Native Hawaiian entities for which the Census Bureau publishes data. The legal entities consist of federally recognized American Indian reservations and off-reservation trust land areas, state-recognized American Indian reservations, and Hawaiian home lands (HHLs)." [More Info](https://www2.census.gov/geo/pdfs/reference/GARM/Ch5GARM.pdf)
...
Also on the state geoportal here: <https://gis.data.ca.gov/maps/waterboards::water-system-outreach-public/explore>
