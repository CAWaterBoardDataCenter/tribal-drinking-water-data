...
server <- function(input, output, session) {
    ...
    # read CA boundary 
    sf_ca_boundary <- st_read('data_processed/ca_boundary.gpkg') %>% st_transform(geographic_crs)
    ...
    sf_tribal_boundaries <- reactive({
        ...
        boundaries <- st_read(paste0('data_processed/', list_tribal_data[[boundary]])) %>% 
    ...
    mh_parks <- st_read('data_processed/ca_mhp_geocode.gpkg') %>% st_transform(geographic_crs)
    ...
    ca_wells <- st_read(here('wells.gpkg')) %>% st_transform(geographic_crs)
    ...
    ca_SSWS <- st_read(here('SSWS.gpkg')) %>% st_transform(geographic_crs)
